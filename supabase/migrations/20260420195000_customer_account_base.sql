...
begin
  ...
  if v_sale.customer_id is null then
    raise exception 'La cancelacion de deuda requiere cliente identificado';
  end if;
  ...
  if v_sale.payment_method <> 'CUENTA_CORRIENTE' then
    raise exception 'Solo se puede acreditar desde una venta a cuenta corriente';
  end if;
  ...
  return public.record_customer_account_entry(
    ...
    coalesce(p_description, 'Cobro por caja'),
