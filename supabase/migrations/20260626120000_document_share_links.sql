create table if not exists public.document_share_links (
  ...
  constraint document_share_links_token_len check (length(token) >= 48)
);

create unique index if not exists document_share_links_one_active_doc
  on public.document_share_links(document_id)
  where enabled;

create index if not exists document_share_links_document_idx
  on public.document_share_links(document_id, enabled, expires_at);

alter table public.document_share_links enable row level security;

drop policy if exists "document_share_links_read_company_member" on public.document_share_links;

create policy "document_share_links_read_company_member"
...
begin
  if v_actor is null then
    raise exception 'Debes iniciar sesion para compartir documentos';
  end if;
...
revoke all on function public.create_document_share_link(uuid, timestamptz) from public;
...
revoke all on function public.revoke_document_share_link(text) from public;
...
revoke all on function public.get_public_document_payload(text) from public;
