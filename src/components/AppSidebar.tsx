...
const navItems = [
  ...
  { title: "Configuración", url: "/settings", icon: Settings, requiresAdmin: true },
];
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-2 py-3">
        {visibleNavItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive ? <span className="absolute bottom-2 left-0 top-2 w-1 rounded-r-full bg-sidebar-primary" /> : null}
      ...
      <div className="border-t border-sidebar-border px-2 py-3">
        ...
        {!collapsed && user ? (
          <p className="mt-2 truncate px-3 text-xs text-sidebar-foreground">{user.email}</p>
        ) : null}
      </div>
    </aside>
  );
