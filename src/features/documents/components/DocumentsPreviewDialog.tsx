...
const HISTORY_TONE_COLORS: Record<string, { bg: string; border: string; text: string; icon: LucideIcon }> = {
  neutral: { bg: "bg-slate-100", border: "border-slate-200", text: "text-slate-600", icon: PlayCircle },
                ...
                <section className="rounded-3xl border border-border/60 bg-card/80 p-5 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="min-w-0 space-y-3">
                      ...
                      <div className="space-y-1">
                        {companySettings.logo_url ? (
                          ...
                          <p className="text-2xl font-semibold tracking-tight">{companySettings.app_name}</p>
                        )}
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-950 px-4 py-3 text-white shadow-sm">
                      ...
                      <p className="mt-1 text-lg font-semibold">{DOC_LABEL[selectedDocument.doc_type]}</p>
                  ...
                  {sourceDocumentLabel ? (
                    <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
                      <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-amber-700">Origen</p>
                      <p className="mt-1 text-sm font-medium text-amber-950">{sourceDocumentLabel}</p>
                    </div>
                  ) : null}
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
                      ...
                      <p className="mt-2 text-lg font-semibold">{selectedDocument.customer_name ?? "Cliente ocasional"}</p>
                    ...
                    <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
                      ...
                      <div className="mt-3 grid grid-cols-[112px_minmax(0,1fr)] gap-y-2 text-sm text-muted-foreground">
                        <p>Fecha:</p>
                        <p className="font-medium text-foreground">{formatIsoDate(selectedDocument.issue_date)}</p>
                        <p>Estado:</p>
                        <p className="font-medium text-foreground">{STATUS_LABEL[selectedDocument.status]}</p>
                        <p>PDV:</p>
                        <p className="font-mono text-foreground">{String(selectedDocument.point_of_sale).padStart(4, "0")}</p>
                        {selectedDocument.payment_terms ? (
                          <>
                            <p>Condición:</p>
                            <p className="font-medium text-foreground">{selectedDocument.payment_terms}</p>
                          </>
                        ) : null}
                        {selectedDocument.doc_type === "REMITO" && selectedDocument.external_invoice_number ? (
                          <>
                            <p>Factura:</p>
                            <p className="font-mono font-medium text-foreground">{selectedDocument.external_invoice_number}</p>
                          </>
                        ) : null}
                        {selectedDocument.salesperson ? (
                          <>
                            <p>Vendedor:</p>
                            <p className="font-medium text-foreground">{selectedDocument.salesperson}</p>
                          </>
                        ) : null}
                        {selectedDocument.valid_until ? (
                          <>
                            <p>Validez:</p>
                            <p className="font-medium text-foreground">{formatIsoDate(selectedDocument.valid_until)}</p>
                          </>
                        ) : null}
                      </div>
                ...
                <section className="rounded-3xl border border-border/60 bg-card/80 p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">Importe y observaciones.</p>
                    </div>
                    <div className="rounded-2xl border border-primary/20 bg-primary/5 px-4 py-3 text-right">
                      <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Total del documento</p>
                ...
                {selectedEvents.length === 0 ? (
                  ...
                  <div className="mt-5 space-y-3">
                    {selectedEvents.map((event, index) => {
                      ...
                      return (
                        <div key={event.id} className="grid grid-cols-[16px_minmax(0,1fr)] gap-3 rounded-2xl border border-border/60 bg-background/80 p-4">
