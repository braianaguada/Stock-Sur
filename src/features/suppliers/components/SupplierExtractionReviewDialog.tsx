      ...
      <div className={isPdf && file ? "grid gap-4 xl:grid-cols-[minmax(20rem,0.85fr)_minmax(0,1.4fr)]" : "space-y-4"}>
        ...
        <div className="min-w-0 space-y-4">
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            ...
            <span>{presentationCount} presentaciones detectadas</span><span>{unknownTaxCount} con IVA no informado</span>
          </div>
          {diagnostics && (
            ...
            {lines.map((line, index) => {
              ...
              return (
                <article key={line.id} className="grid gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_11rem_10rem_10rem_auto] lg:items-end">
                  ...
                  <div className="space-y-1">
                    <Label>IVA</Label>
                    <Select value={line.tax_treatment} onValueChange={(value: ExtractionReviewLine["tax_treatment"]) => onLineChange(line.id, { tax_treatment: value })}><SelectTrigger aria-label={`IVA del producto ${index + 1}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="INCLUDED">Incluido</SelectItem><SelectItem value="EXCLUDED">Más IVA</SelectItem><SelectItem value="UNKNOWN">No informado</SelectItem></SelectContent></Select>
                  </div>
                  ...
                  {(line.source_page || typeof line.confidence === "number") && <p className="text-xs text-muted-foreground lg:col-span-5">Origen: {line.source_page ? `página ${line.source_page}` : "archivo"}{typeof line.confidence === "number" ? ` · confianza ${Math.round(line.confidence * 100)}%` : ""}</p>}
                </article>
              );
            })}
          </div>
