  ...
  return <Dialog open={props.open} onOpenChange={props.onOpenChange}>
    <DialogContent className="max-w-lg">
      ...
      <label className="grid min-h-40 cursor-pointer place-items-center rounded-xl border border-dashed border-primary/40 bg-primary/5 p-6 text-center hover:bg-primary/10">
        <span>{loading ? <Loader2 className="mx-auto mb-3 h-7 w-7 animate-spin" /> : <Camera className="mx-auto mb-3 h-7 w-7" />}<b className="block">{loading ? "Interpretando remito..." : "Tomar foto o subir archivo"}</b><small className="mt-2 block text-muted-foreground">JPG, PNG, WEBP o PDF, hasta 8 MB. Los fragmentos dudosos no se copiarán.</small></span>
        <input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" disabled={loading} onChange={(event) => { void scan(event.target.files?.[0]); event.currentTarget.value = ""; }} />
      </label>
