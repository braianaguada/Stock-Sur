...
export function DashboardHighlights({ dashboard }: { dashboard: DashboardInsights }) {
  ...
  return (
    <section className="dashboard-panel flex h-full flex-col" aria-labelledby="dashboard-highlights-title">
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        {payments.length ? payments.map((item) => (
          <div key={item.method}>
            ...
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-gradient-to-r from-primary to-[hsl(var(--brand-cyan))]" style={{ width: `${Math.max(4, item.total / max * 100)}%` }} /></div>
          </div>
