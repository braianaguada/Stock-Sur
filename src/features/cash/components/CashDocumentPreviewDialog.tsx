...
type CashDocumentPreviewDialogProps = {
  ...
  detailSale: CashMovementRow | null;
  ...
  canAttachReceipt: (sale: CashMovementRow) => boolean;
  canCancelSale: (sale: CashMovementRow) => boolean;
  onAssignReceipt: (sale: CashMovementRow) => void;
...
export function CashDocumentPreviewDialog(props: CashDocumentPreviewDialogProps) {
  ...
  const isReturnDocument = linkedDocument?.doc_type === "REMITO_DEVOLUCION";
  const documentLabel = isReturnDocument ? "Remito devolucion" : "Remito";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex flex-col h-[min(92vh,920px)] max-w-[min(97vw,1520px)] overflow-hidden border-border/60 bg-background/95 shadow-2xl backdrop-blur-xl">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>Documento asociado al movimiento y su trazabilidad.</DialogDescription>
        </DialogHeader>
        ...
        {detailSale && linkedDocument ? (
          <div className="grid gap-4 rounded-2xl border border-border/60 bg-card/80 p-4 shadow-sm md:grid-cols-4">
            ...
            <div><p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Importe</p><p className={`mt-1 font-semibold ${detailSale.display_amount < 0 ? "text-destructive" : ""}`}>{currency.format(Number(detailSale.display_amount))}</p></div>
          </div>
        ) : null}
          ...
          {linkedDocument ? (
            <>
                  ...
                  <div className="border-b border-border/60 px-5 py-4 sm:px-6">
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div className="min-w-[180px] border-l border-border/60 pl-4 text-right">
                          ...
                          <p className="mt-1 text-lg font-semibold text-foreground">{documentLabel}</p>
                  ...
                  <section className="rounded-2xl border border-border/60 bg-card/90 p-5 shadow-sm">
                    <div className="flex flex-wrap items-end justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">Detalle principal del movimiento asociado.</p>
                      </div>
              ...
              <aside className="min-h-0 overflow-y-auto pr-1 pb-2 [scrollbar-gutter:stable] 2xl:min-w-[380px]">
                <section className="rounded-2xl border border-border/60 bg-card/90 p-5 shadow-sm">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Trazabilidad del movimiento y del documento.</p>
            ...
            <div className="rounded-2xl border border-dashed border-border/60 bg-card/60 p-6 text-sm text-muted-foreground">
              {detailSale?.status === "ANULADA"
                ? "Movimiento anulado: no hay documento asociado visible."
