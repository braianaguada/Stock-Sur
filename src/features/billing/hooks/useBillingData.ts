  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await billingDb
        ...
        .select("id, company_id, source_type, source_id, source_remito_id, document_kind, invoice_type, fiscal_status, provider, environment, receiver_name, receiver_doc_type, receiver_doc_number, receiver_tax_condition, subtotal, discount_total, tax_total, total, point_of_sale, voucher_number, voucher_full_number, cae, cae_expires_at, authorized_at, error_message, created_at, updated_at")
        .eq("company_id", companyId!)
