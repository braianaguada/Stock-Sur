...
export function buildBillingPrintHtml({ document, lines, remito, relatedDocument, qrDataUrl }: BuildBillingPrintHtmlParams) {
  ...
  const documentCode = isCreditNote ? "008" : document.invoice_type === "FACTURA_A" ? "001" : "006";
...
<body>
  ...
  <main class="page">
    ${document.fiscal_status !== "AUTHORIZED" || !document.cae ? `
      <div class="warning">${isInvoiceADraft ? "BORRADOR - No valido como comprobante fiscal. Factura A en preparacion." : "Este comprobante no esta autorizado fiscalmente. No debe entregarse como factura fiscal."}</div>
    ` : ""}
    <section class="header">
      ...
      <div class="letter-box">
        ...
        <div class="letter-caption">Cod. ${documentCode}</div>
      </div>
    ...
    <section class="section section-grid">
      ...
      <div class="origin-box">
        ...
        ${isCreditNote ? `
          <p><span class="label">Factura asociada:</span> ${escapeHtml(originalVoucher)}</p>
          <p><span class="label">Fecha factura:</span> ${formatDate(relatedDocument?.voucher_date)}</p>
        ` : ""}
    ...
    ${isCreditNote ? `
      <section class="section">
        ...
        <p>Factura B: ${escapeHtml(originalVoucher)}</p>
