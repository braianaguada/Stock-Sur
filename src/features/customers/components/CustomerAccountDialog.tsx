...
type Props = {
  ...
  isLoading: boolean;
  onRegisterCredit: () => void;
  onOpenChange: (open: boolean) => void;
};

export function CustomerAccountDialog({ open, customer, summary, movements, isLoading, onRegisterCredit, onOpenChange }: Props) {
  return (
    <EntityDialog open={open} onOpenChange={onOpenChange} title={customer ? `Cuenta corriente · ${customer.name}` : "Cuenta corriente"} contentClassName="sm:max-w-4xl">
      <div className="space-y-4">
        ...
        <div className="flex justify-end">
          <Button onClick={onRegisterCredit} disabled={!customer}>Registrar cobro</Button>
        </div>
        <DataTable
          columns={[
            { accessorKey: "business_date", header: () => "Fecha" },
            { accessorKey: "entry_type", header: () => "Tipo", cell: ({ row }) => <Badge variant={row.original.entry_type === "DEBIT" ? "destructive" : "default"}>{row.original.entry_type}</Badge> },
